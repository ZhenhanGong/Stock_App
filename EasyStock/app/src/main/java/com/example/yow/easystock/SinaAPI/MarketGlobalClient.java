 ...
 */
public class MarketGlobalClient {
    ...
    private static MarketGlobalClient mInstance;

    private MarketGlobalClient(){
    ...
    public synchronized static MarketGlobalClient getInstance() {

        if(mInstance != null) {
            return mInstance;
        }
        return new MarketGlobalClient();
    }

    public List<MarketGlobalInfo> getStockInfo(String[] stockCodes) throws HttpException, IOException, ParseStockInfoException{
        ...
        List<MarketGlobalInfo> list = parseSinaStockInfosFromReader(bReader);
        bReader.close();
        method.releaseConnection();
    ...
    private List<MarketGlobalInfo> parseSinaStockInfosFromReader(BufferedReader reader) throws IOException, ParseStockInfoException {

        ArrayList<MarketGlobalInfo> list = new ArrayList<MarketGlobalInfo>(10);
        String sourceLine = null;

        while((sourceLine = reader.readLine()) != null) {
            list.add(MarketGlobalInfo.parseStockInfo(sourceLine));
            //	break;
        }
